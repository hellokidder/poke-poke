...
| 编号 | 用例名称 | 前置条件 | 操作步骤 | 预期结果 |
|------|---------|---------|---------|---------|
| TC-N-010 | Running → Success | 会话状态为 Running | upsert 状态为 Success | 状态更新为 Success |
...
| TC-N-013 | Success → Running | 会话状态为 Success | upsert 状态为 Running | 状态更新为 Running |
| TC-N-014 | Running → Running | 会话状态为 Running | upsert 仍为 Running | 状态保持 Running |
| TC-N-015 | Success → Success | 会话状态为 Success | upsert 仍为 Success | 状态保持 Success |
...
| TC-N-018 | prev_status 返回值 | 会话状态为 Running | upsert 更新为 Success | `prev_status == Some(Running)` |
...
| 编号 | 用例名称 | 前置条件 | 操作步骤 | 预期结果 |
|------|---------|---------|---------|---------|
| TC-N-060 | 过期终态被删除 | Success 会话 updated_at 为 25 小时前 | 调用 cleanup_expired(24) | 该会话被删除 |
| TC-N-061 | Running 不被删除 | Running 会话 updated_at 为 100 小时前 | 调用 cleanup_expired(24) | 会话保留（Running 永不清除） |
| TC-N-062 | Pending 不被删除 | Pending 会话 updated_at 为 100 小时前 | 调用 cleanup_expired(24) | 会话保留 |
| TC-N-063 | 未过期终态保留 | Success 会话 updated_at 为 1 小时前 | 调用 cleanup_expired(24) | 会话保留 |

### 1.7 僵死会话探活（lib.rs 高频巡检线程）

> 探活逻辑已从 `sessions.rs` 的 `reap_stale_sessions()` 移至 `lib.rs` 的高频巡检线程（每 5s 检测，grace period 2 次失败才 remove）。

| 编号 | 用例名称 | 前置条件 | 操作步骤 | 预期结果 |
|------|---------|---------|---------|---------|
| TC-N-070 | TTY 不存在 → 移除 | Running 会话 tty="/dev/ttys999"（不存在的路径） | 巡检线程连续 2 次检测到 TTY 不存在 | 会话被 remove，从列表中移除 |
| TC-N-071 | 非 Running 跳过 | Success 会话 tty 路径不存在 | 巡检线程执行 | 不受影响 |
| TC-N-072 | tty 为 None 跳过 | Running 会话无 tty | 巡检线程执行 | 不受影响 |
| TC-N-073 | tty 为空字符串跳过 | Running 会话 tty="" | 巡检线程执行 | 不受影响 |
| TC-N-074 | Grace period — 单次失败不移除 | Running 会话 tty 不存在 | 巡检线程首次检测到 | 会话保留（需连续 2 次失败才移除） |
...
| TC-H-006 | status 解析 — success | - | POST status="success" | 会话状态为 Success |
| TC-H-007 | status 解析 — failed | - | POST status="failed" | 会话状态映射为 Success（Failed 已移除） |
...
| 编号 | 用例名称 | 前置条件 | 操作步骤 | 预期结果 |
|------|---------|---------|---------|---------|
| TC-H-020 | Running → Success 弹窗 | 已有 Running 会话 | POST status="success" | 弹窗弹出 + 播放提示音 |
| TC-H-021 | Running → Pending 弹窗 | 已有 Running 会话 | POST status="pending" | 弹窗弹出（需要用户审批） |
| TC-H-022 | Pending → Running 关闭弹窗 | 有 Pending 弹窗 | POST status="running" | 已有弹窗关闭（用户恢复操作） |
| TC-H-023 | Success → Running 关闭弹窗 | 有 Success 弹窗 | POST status="running" | 已有弹窗关闭 |
| TC-H-024 | Running → Running 无弹窗 | 已有 Running 会话 | POST 仍为 running | 不弹窗（状态未变化） |
| TC-H-025 | 新会话直接 Success | 无该 task_id | POST 新会话 status="success" | 弹窗弹出（直接创建为终态） |
...
| TC-K-036 | Cursor stop (aborted) | Cursor 运行中 | Cursor 发送 stop, status=aborted | POST status=success（Failed 已移除，统一映射为 Success） |
...
| TC-FP-012 | isActive — 只有 running/pending 为活跃 | status = "success" | `false`（Failed 已移除，终态仅 Success） |
...
| TC-FP-022 | 活跃会话计数 | 1 个 Running + 1 个 Pending + 1 个 Success | 打开面板 | 标题显示 "2 个活跃会话" |
| TC-FP-023 | 点击会话跳转 | 存在会话 | 点击会话行 | 调用 open_task_source，聚焦对应终端 |
| TC-FP-024 | 删除非活跃会话 | 存在 Success 会话 | 点击删除按钮 | 会话被移除 |
| TC-FP-025 | 活跃会话无删除按钮 | 存在 Running 会话 | 查看面板 | 无删除按钮 |
...
| TC-IC-010 | 动画 — success | status="success" | CSS bounce 弹跳动画 |
...
| TC-E2E-011 | Cursor 中止 | 1. Cursor 运行中<br>2. 用户中止（stop, status=aborted） | 面板显示 Success（Failed 已移除，统一映射为 Success），弹窗弹出 |
...
| 编号 | 场景 | 步骤 | 预期结果 |
|------|------|------|---------|
| TC-E2E-050 | 应用重启后恢复 | 1. 有多条会话<br>2. 退出重启 Poke Poke | 面板中会话列表恢复（从 ~/.pokepoke/sessions.json 加载） |
| TC-E2E-051 | 过期会话清理 | 1. 有 25 小时前的 Success 会话<br>2. 等待清理定时器 | 过期会话被自动删除 |
| TC-E2E-052 | 僵死会话检测 | 1. CC 会话 Running<br>2. 关闭终端（TTY 设备文件消失） | 巡检线程（每 5s）连续 2 次检测到 TTY 不存在后，会话被移除 |
