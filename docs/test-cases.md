...
| TC-N-019b | 老数据 serde alias（**单向：仅读老数据时生效**） | sessions.json 历史写入 `"success"` / `"failure"` | 加载文件 | 分别反序列化为 Idle / LastFailed；新版本写入仍用 `"idle"` / `"last_failed"` |
...
> ⚠️ `popup_timeout` 设计已撤销，`Settings` 不再有此字段。当前 popup 只在以下情形关闭：
> 1. 用户聚焦到对应终端（TC-P-013）
> 2. session 进入新一轮（`* → Running`，由 `should_close_popup` 决定）
> 3. session 被探活线程 reap 时一并关闭（见 `lib.rs` `remove_session_with_cleanup`）
...
> ⚠️ 原 `TC-S-003 默认弹窗超时` 已删除：`popup_timeout` 字段不复存在（见 §4.2 备注）。
...
> ⚠️ 原 `TC-FS-023 切换弹窗超时` 已删除：UI 上无此设置项，`popup_timeout` 字段不复存在。
...
| 编号 | 用例名称 | 输入 | 预期结果 |
|------|---------|------|---------|
| TC-IC-001 | Claude Code 颜色 | source="claude-code" | `#D97757`（橙） |
| TC-IC-002 | Cursor 颜色 | source="cursor" | `#00D4AA`（青） |
| TC-IC-003 | Codex 颜色 | source="codex" | `#10A37F`（绿） |
...
| TC-CLI-003 | 完成会话 | 存在 Running 会话 | `pokepoke done <task_id>` | 会话变为 Idle，弹窗弹出 |
