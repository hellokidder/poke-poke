...
|------|---------|---------|---------|---------|
| TC-N-001 | 新任务插入 | TaskStore 为空 | 调用 `upsert_task` 传入新 task_id | `is_new == true`，`read == false`，任务列表长度为 1 |
...
| 编号 | 用例名称 | 前置条件 | 操作步骤 | 预期结果 |
|------|---------|---------|---------|---------|
| TC-N-010 | Running → Success | 任务状态为 Running | upsert 状态为 Success | `read = false`（终态转换标记未读，需要用户关注） |
| TC-N-011 | Running → Failed | 任务状态为 Running | upsert 状态为 Failed | `read = false` |
| TC-N-012 | Running → Pending | 任务状态为 Running | upsert 状态为 Pending | `read = false`（需要用户批准权限） |
| TC-N-013 | Pending → Running | 任务状态为 Pending | upsert 状态为 Running | `read = true`（用户已恢复操作，自动已读） |
| TC-N-014 | Success → Running | 任务状态为 Success | upsert 状态为 Running | `read = true`（新一轮开始，自动已读） |
| TC-N-015 | Failed → Running | 任务状态为 Failed | upsert 状态为 Running | `read = true` |
| TC-N-016 | Running → Running | 任务状态为 Running | upsert 仍为 Running | `read` 值不变（无匹配规则） |
| TC-N-017 | Success → Success | 任务状态为 Success | upsert 仍为 Success | `read = false`（终态 → 终态仍标记未读，因为 `is_terminal() && !prev.is_terminal()` 不匹配，实际 read 不变） |
...
### 1.3 未读计数 (unread_count)

| 编号 | 用例名称 | 前置条件 | 操作步骤 | 预期结果 |
|------|---------|---------|---------|---------|
| TC-N-030 | 终态未读计入 | 一条 Success + read=false | 查询 unread_count | 返回 1 |
| TC-N-031 | Pending 未读计入 | 一条 Pending + read=false | 查询 unread_count | 返回 1 |
| TC-N-032 | Running 不计入 | 一条 Running + read=false | 查询 unread_count | 返回 0（Running 永不计入未读） |
| TC-N-033 | 已读不计入 | 一条 Success + read=true | 查询 unread_count | 返回 0 |
| TC-N-034 | 混合场景 | 3 条任务：Running(未读) + Success(未读) + Pending(已读) | 查询 unread_count | 返回 1（仅 Success 未读计入） |

### 1.4 标记已读

| 编号 | 用例名称 | 前置条件 | 操作步骤 | 预期结果 |
|------|---------|---------|---------|---------|
| TC-N-040 | mark_read 成功 | 存在 id="xxx" 的任务 | 调用 mark_read("xxx") | 返回 true，该任务 read=true |
| TC-N-041 | mark_read 不存在 | 无 id="yyy" | 调用 mark_read("yyy") | 返回 false |
| TC-N-042 | mark_all_read 范围 | Running + Success + Pending 各一条 | 调用 mark_all_read | Success 和 Pending 变为已读，Running 不变 |
...
| 编号 | 用例名称 | 前置条件 | 操作步骤 | 预期结果 |
|------|---------|---------|---------|---------|
| TC-N-070 | TTY 不存在 → Failed | Running 任务 tty="/dev/ttys999"（不存在的路径） | 调用 reap_stale_sessions | 状态变为 Failed，message="Session lost"，read=false |
...
### 2.4 POST /notifications/{id}/read

| 编号 | 用例名称 | 前置条件 | 操作步骤 | 预期结果 |
|------|---------|---------|---------|---------|
| TC-H-040 | 标记已读 | 存在未读任务 id="xxx" | POST /notifications/xxx/read | 响应 200，任务变为已读 |
| TC-H-041 | ID 不存在 | 无 id="yyy" | POST /notifications/yyy/read | 响应 404 |
...
| TC-FP-021 | 任务排序 | 有多条任务 | 打开面板 | 按创建时间倒序排列（最新的在顶部） |
...
| TC-T-005 | 未读计数 tooltip | 有 3 条未读 | 悬停托盘图标 | tooltip 显示含 "3" 的未读提示 |
...
| TC-CLI-006 | 清除已读 | 有未读通知 | `pokepoke clear` | 全部标记为已读 |
...
| TC-E2E-003 | CC 权限通过后恢复 | 1. 状态为 Pending<br>2. 用户审批后 CC 继续（触发 UserPromptSubmit） | 弹窗关闭，状态变回 Running，自动标记已读 |
